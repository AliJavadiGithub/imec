...
def main():
    ...
    # ۲. بارگذاری فایل‌های PCD
    all_files = glob.glob(os.path.join(map_dir, "*.pcd"))
    pcd_files = sorted(all_files, key=extract_timestamp)

    # ۳. تنظیمات بصری‌ساز
    vis = o3d.visualization.Visualizer()
    vis.create_window(window_name="Tracking Overlay Recorder", width=1280, height=720, visible=True)
    
    pcd = o3d.io.read_point_cloud(pcd_files[0])
    vis.add_geometry(pcd)

    # تنظیمات دوربین و ظاهر
    opt = vis.get_render_option()
    opt.background_color = np.asarray([0, 0, 0])
    opt.point_size = 2.0
    
    # آماده‌سازی ویدیو رایتر
    ...
    for i, pcd_file in enumerate(pcd_files):
        ...
        # اضافه کردن Overlay تراکینگ
        if i in tracking_dict:
            for det in tracking_dict[i]:
                pos = det['position']
                color = [0, 1, 0] if det.get('status') == "MOVING" else [1, 1, 0]
    ...
    vis.destroy_window()
    print(f"\nSuccessfully saved tracking video as {video_output}")
